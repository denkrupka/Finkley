...
**Интеграционные RLS/RPC тесты против staging Supabase** периодически
падают с `AuthApiError: Database error creating new user` (rate-limit
auth API). Они маскируют реальные регрессии в pure unit-тестах. Если
ты гоняешь полный `pnpm test` локально для smoke-check после большого
рефактора и не нужны DB-тесты — запускай так:

```bash
SKIP_INTEGRATION_TESTS=1 pnpm test
```

Это скипнет все `tests/unit/*-rpc.test.ts` / `*-rls.test.ts` /
`*-trigger.test.ts` через `describe.skipIf(shouldSkip)` (см.
`apps/web/tests/unit/_helpers.ts`). В CI они идут отдельным
workflow `integration-tests.yml` с явно заданными env vars.
