// One-shot helper: set a single secret on staging + prod via Management API.
// Usage: node scripts/set-secret.mjs <NAME> <VALUE>
...
if (!NAME || !VALUE) {
  console.error('Usage: node scripts/set-secret.mjs <NAME> <VALUE>')
  process.exit(1)
}
...
for (const t of targets) {
  ...
  console.log(`[${t.label}] ✓ ${NAME} set on ${ref}`)
}
