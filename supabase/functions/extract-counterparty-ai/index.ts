...
const PARSE_PROMPT = `Ты — опытный bookkeeping-аналитик с 8+ годами работы с польскими банковскими выписками (mBank, ING, Santander, PKO BP, Revolut). Знаешь как банки форматируют описания — где имя контрагента, а где назначение/код перевода.

GROUNDING (КРИТИЧНО): извлекай имя продавца ТОЛЬКО если оно ЯВНО видно в строке. Если строка — назначение платежа без имени ("Zwrot pożyczki", "Transfer to phone number") — возвращай null. НИКОГДА не угадывай имя из контекста.
ANTI-FLUFF: не объясняй, не комментируй, только JSON.

Задача: для каждой банковской транзакции извлеки имя контрагента/продавца если оно ЕСТЬ в строке. Если нет — null.
        ...
        messages: [
          {
            role: 'system',
            content:
              'Ты опытный bookkeeping-аналитик. Возвращаешь СТРОГО валидный JSON без markdown-fences. Не выдумываешь имена.',
          },
