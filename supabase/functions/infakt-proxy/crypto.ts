/**
 * AES-256-GCM шифрование inFakt api_token (см. ADR-013 §C).
 * Идентично паттерну wfirma-proxy/crypto.ts, отдельный ключ INFAKT_SECRETS_KEY.
 */

const KEY_NAME = 'INFAKT_SECRETS_KEY'
...
async function getKey(): Promise<CryptoKey> {
  ...
  if (bin.length !== 32) throw new Error(`${KEY_NAME} must decode to 32 bytes`)
