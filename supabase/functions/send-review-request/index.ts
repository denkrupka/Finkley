/**
 * send-review-request — cron КАЖДУЮ МИНУТУ, шлёт клиенту запрос на отзыв
 * сразу после оплаты визита (near-real-time, в UI выглядит как «мгновенно»).
 *
 * Кому шлём:
 *   - Визиты со status='paid' за последние 24 часа (safety window — на случай
 *     если cron был простой). 99% случаев визит обработается в первой
 *     же итерации после оплаты.
 *   - У клиента есть email или phone.
 *   - На этот visit ещё не было review_request (anti-dup по visit_id).
 *
 * Что делаем:
