...
const SYSTEM_PROMPT = `Ты обрабатываешь фото бумажного чека из салона красоты или магазина в Польше/Европе.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!match) return jsonResponse({ error: 'no_json_in_response', raw: text.slice(0, 200) }, 502)
