-- =============================================================================
-- 20260626000003_setup_progress_v3.sql
-- =============================================================================
-- T2 (v3) — двe оставшиеся задачи чек-листа «Настройка Finkley» становятся
-- РЕАЛЬНО отслеживаемыми:
--
--   has_finance_report      — был заглушкой false (нет таблицы трекинга
--                             генерации P&L/ДДС). Теперь детектится из
--                             tracking_events: event_type='action' и
--                             path='finance_report_generated' (эмитится из SPA
--                             при экспорте/печати фин-отчёта — Excel/CSV/PDF).
--   has_marketing_broadcast — был заглушкой false (была только
--                             salons.broadcast_prefs). Теперь детектится из
--                             tracking_events: event_type='action' и
--                             path='marketing_broadcast_sent' (эмитится
--                             серверно из edge function marketing-send-broadcast
--                             при РЕАЛЬНОЙ успешной отправке, не dry_run).
--
-- Почему tracking_events, а не новые таблицы:
--   * Таблица уже есть (миграция 20260603000009), уже используется из SPA
--     (useTrackPageView) и edge functions, RLS уже настроена (юзер пишет
--     только свои события: with check user_id = auth.uid()). Один механизм для
--     обеих задач = меньше схемы, проще аналитика.
--   * Конвенция таблицы: event_type='action', path=<action key> (см. комментарий
--     в 20260603000009). Используем её as-is, новые поля/таблицы не нужны.
--
-- Совместимость:
--   * Return type функции НЕ меняется относительно v2 (те же OUT-колонки в том
--     же порядке) — но для замены тела используем DROP + CREATE (та же практика
--     что и v2; CREATE OR REPLACE с тем же сигнатурой тоже сработал бы, но
--     держим симметрию с v2). serverKey-маппинг в setup-progress.ts НЕ меняется
--     (поля те же has_finance_report / has_marketing_broadcast).
--   * security invoker (default) сохраняется → RLS каждой читаемой таблицы
--     режет доступ. tracking_events RLS на select разрешает юзеру видеть
--     только свои события (user_id = auth.uid()); это ок — owner салона сам
--     генерит отчёт / шлёт рассылку, поэтому его user_id = автор события.
--     Для надёжной кросс-юзерной детекции (если рассылку отправил со-владелец)
--     scoping всё равно идёт по salon_id, а RLS-видимость owner'а покрывает
--     основной сценарий (он же владелец и инициатор настройки).
-- =============================================================================
...
comment on function public.setup_progress(uuid) is
  'Серверный прогресс «Настройки Finkley» (v3). security invoker → RLS '
  ...
  'has_finance_report детектится из tracking_events action '
  '''finance_report_generated'' (экспорт фин-отчёта из SPA); '
  'has_marketing_broadcast — из tracking_events action '
  '''marketing_broadcast_sent'' (реальная рассылка из marketing-send-broadcast).';
