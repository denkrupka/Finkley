-- =============================================================================
-- 20260505000007_dashboard_rpcs.sql
-- =============================================================================
-- RPC-функции для дашборда (вызываются с клиента через .rpc())
-- Используют security invoker (default) — RLS работает автоматически
-- =============================================================================

-- KPI дашборда: выручка, расходы, прибыль, кол-во визитов за период
...
-- Топ-N мастеров по выручке за период
...
language sql stable as $$
  select
    s.id,
    s.full_name,
    coalesce(sum(v.amount_cents), 0) as revenue_cents
  from staff s
...
-- Топ-N услуг по выручке за период
...
-- Выручка по дням (для бар-чарта дашборда)
