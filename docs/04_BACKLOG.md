...
**Что в проде:** Edge Function `public-api` (registry-driven, 44 ресурса: визиты/доходы/расходы/клиенты/услуги/мастера/выплаты/склад/контрагенты/конкуренты/касса(read)/банк(read) + 12 аналитических RPC). Аутентификация по `fnk_live_` ключу (SHA-256 → `api_keys`), scopes read/write, жёсткий salon-scoping (service-role), discovery-эндпоинт `GET /v1`, `/me`, пагинация/фильтры/диапазоны дат. `config.toml`: `verify_jwt=false`. Публичная страница `/docs/api` (`ApiDocsPage.tsx`) читает живой каталог → доки не расходятся с API. Кнопка из Настройки → API, curl-примеры исправлены на реальный URL. Тесты `deno test supabase/functions/public-api/` (30: lib/registry/smoke). ADR-032. read-only в v1: касса/склад-движения/банк/parent-scoped/payouts(кроме статуса); исключены 3 RPC с `auth.uid()`-guard и глобальный `media_posts`.
