...
- **Credentials в localStorage в plain-text.** localStorage доступен любому
  JS на том же origin'е — XSS на странице salon = утечка. Митигация:
  (1) Origin Finkley находится за strict CSP (нет inline-scripts кроме
  hCaptcha/Stripe whitelist); (2) consume one-shot — окно exposure
  ~5 секунд между submit'ом онбординга и mount'ом connect-dialog'а;
  (3) localStorage scope изолирован per-salon (другой салон не прочитает).
  Если в будущем будем поддерживать кросс-table embed виджеты —
  переход на in-memory state через router state mandatory.
- Credentials видны в DevTools → Application → Local Storage в течение
  тех же ~5 секунд. Не для shared workstation сценариев.
