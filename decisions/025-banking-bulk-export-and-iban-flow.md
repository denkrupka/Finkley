...
### 3a. Mismatch-модалка при разнице сумм tx/сущности

Когда юзер привязывает банковскую tx к расходу/визиту/доходу с другой
суммой — открывается `AmountMismatchDialog` (миграция не нужна, только UI):

- **tx < remaining** (3 опции):
  - «Частичная оплата» — для expense создаём
    `expense_payment_installment` (триггер пересчитает paid_amount_cents),
    для visit/other_income просто линкуем (частичная оплата у них не
    моделируется).
  - «Изменить сумму ↓» — `entity.amount_cents = alreadyPaid + txAmt`,
    для expense ещё сброс paid_amount_cents = null (полностью оплачено).
  - «Отмена».
- **tx > remaining** (2 опции): «Увеличить сумму ↑» / «Отмена»
  (переплату не моделируем).
- Учитывает уже-оплаченные части: paid=50 + tx=70 на expense=100 →
  tx > remaining(50) → 2 опции вместо 3.

### 3b. Журнал частичных оплат (expense_payment_installments)

Миграция `20260526114212_expense_payment_installments.sql`:

- Таблица `expense_payment_installments` (id, expense_id FK cascade,
  paid_at, amount_cents, payment_method, cash_register_id,
  bank_transaction_id FK on-delete-set-null, comment, created_by).
- RLS через `expense_id → expense.salon_id → salon_members`.
- Trigger `recalc_expense_paid_amount` (after insert/update/delete) —
  пересчитывает `expenses.paid_amount_cents = SUM(installments)`. Если
  SUM >= amount → NULL (legacy «полностью оплачено» по семантике
  `effectivePaidCents()`).

В `ExpenseFormModal` edit-mode — компонент `InstallmentsList`: список
«Дата · Сумма · Касса/Банк» + кнопка ✕ удалить (откат через триггер).
Из mismatch-модалки при «Частичная оплата» — installment создаётся с
`bank_transaction_id = tx.id` (для traceability).

### 3c. Auto-extract counterparty из description

В `banking-sync` edge function — `extractCounterpartyFromDescription`
heuristic 3 паттерна:

- Leading UPPERCASE-words 1..4 (PL POS-формат: «ROSSMANN 21 POZNAN POL»
  → «ROSSMANN 21») с обрезкой гео-шума.
- Домены «APPLE.COM», «GOOGLE.PL».
- Первое CamelCase-слово ≥4 символов («Revolut», «Enea», «Spotify»).

Применяется только при `counterpartyName(t) == null` — не переопределяет
EB-данные где они есть. LLM (Groq) fallback можно добавить отдельной
итерацией если эвристика слабовата.

### 4. Embed страниц в Link-модалки (UX-симметрия)
