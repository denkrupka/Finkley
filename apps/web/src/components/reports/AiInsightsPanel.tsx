import { AlertCircle, ArrowRight, Lightbulb, Loader2, Sparkles } from 'lucide-react'
...
/**
 * Жёлтая плашка над аналитической вкладкой /reports с AI-выводами
 * по данным отчёта. У каждого инсайта — кнопка «Что с этим делать?»,
 * которая сохраняет prompt в sessionStorage и переходит на /ai —
 * AIAssistantPage подхватывает префилл и сразу отправляет в чат.
 *
 * Использование:
...
export function AiInsightsPanel({ kind, payload }: { kind: InsightKind; payload: unknown }) {
  ...
  // Если данных мало — не дёргаем AI (нечего анализировать).
  const enabled = Boolean(salonId) && payload != null
  const {
    data: insights = [],
    isLoading,
    error,
