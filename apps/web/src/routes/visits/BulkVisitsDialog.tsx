...
/**
 * Multi-row форма ввода визитов: одна дата (default — сегодня) + до 10 строк
 * (мастер / услуга / сумма / оплата). Сохраняет всё одной батч-операцией.
 *
 * Юзер-кейс: вечером после рабочего дня внести все визиты разом.
 */
export function BulkVisitsDialog({
  open,
  onOpenChange,
  salonId,
  currency,
  ...
  // Валидные строки = есть staff_id + amount > 0. Service опционален.
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:!max-w-[640px]">
        <DialogHeader>
          <DialogTitle>{t('visits.bulk.title')}</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-3 px-5 pb-2 pt-2">
          ...
          <div className="border-border bg-muted/20 text-muted-foreground grid grid-cols-[1fr_1fr_90px_90px_36px] gap-2 rounded-md border px-2 py-2 text-[11px] font-bold uppercase tracking-wider sm:grid-cols-[1.2fr_1.5fr_120px_120px_36px]">
            <span>{t('visits.bulk.staff')}</span>
            <span>{t('visits.bulk.service')}</span>
          ...
          <div className="flex max-h-[55vh] flex-col gap-2 overflow-y-auto">
            {rows.map((r) => (
              <div
                key={r.id}
                className="grid grid-cols-[1fr_1fr_90px_90px_36px] items-center gap-2 sm:grid-cols-[1.2fr_1.5fr_120px_120px_36px]"
              >
                ...
                <div className="border-brand-yellow-deep bg-brand-yellow flex h-9 items-center rounded-md border-[1.5px] px-2">
                  <input
                    type="number"
                    inputMode="decimal"
                    step="any"
                  ...
                  />
                  <span className="num text-brand-navy text-xs">{currencySymbol}</span>
                </div>
                <Select
                ...
                <button
                  ...
                  className="text-muted-foreground hover:text-destructive grid size-9 place-items-center rounded-md disabled:opacity-30"
                >
        ...
        <DialogFooter className="px-5">
          ...
          <Button onClick={submit} disabled={submitting || validRows.length === 0}>
