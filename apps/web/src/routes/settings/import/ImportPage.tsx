...
/**
 * Платформо-специфичные шаблоны: точные имена колонок CSV-экспортов из
 * Booksy/Fresha/Treatwell. Если матчим >=3 колонки — считаем платформу
 * детектированной и применяем шаблон вместо общей эвристики.
 */
const PLATFORM_TEMPLATES: Record<string, Record<string, DomainField>> = {
  booksy: {
...
function guessField(header: string): DomainField {
  ...
  if (/(date|время|дата|day|когда|godzina)/i.test(h)) return 'visit_at'
  if (/(price|amount|sum|total|итого|сумма|стоимость|cena|kwota)/i.test(h)) return 'amount'
  if (/(phone|tel|телефон|номер)/i.test(h)) return 'client_phone'
  if (
    /(client|customer|клиент|имя|name|klient)/i.test(h) &&
    !/staff|master|мастер|pracownik/i.test(h)
  )
    return 'client_name'
  if (/(staff|master|мастер|specialist|pracownik|therapist|team)/i.test(h)) return 'staff_name'
  if (/(service|услуга|product|treatment|usługa)/i.test(h)) return 'service_name'
  if (/(payment|paid|способ|оплата|method|płatnoś)/i.test(h)) return 'payment_method'
  if (/(comment|note|комментарий|примечание|notatka|notes)/i.test(h)) return 'comment'
  return 'skip'
}

/**
 * Детектируем платформу: считаем сколько headers матчатся точным
 * шаблоном. Победитель >=3 совпадений выигрывает; иначе fallback на
 * generic guessField.
 */
function buildMapping(headers: string[]): Record<number, DomainField> {
  ...
  function onFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      try {
        ...
        setParsed(result)
        setMapping(buildMapping(result.headers))
        setProgress(null)
      } catch (err) {
