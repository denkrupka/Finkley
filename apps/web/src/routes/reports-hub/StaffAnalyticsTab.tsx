 ...
 *   - Выручка (визиты + retail + чаевые отдельной колонкой)
 *   - Визитов, уникальных клиентов, Rebook%, Utilization%
 *   - 6m выручка + стаж работы
 *
 * Источник: RPC staff_performance_advanced (migration 20260515000013 +
 * 20260521000011 для чаевых).
 */
...
function PerformanceSubTab({
  ...
  // bug 64d0bbf8 — Отток + Скоринг теперь считаются server-side в RPC
  // staff_performance_advanced (миграция 20260530000002_staff_churn_scoring).
  // RPC возвращает churn_pct (0..100) и scoring (decimal, ~0..5).
  // Никаких client-side выборок visits больше нет.
            ...
            <table className="w-full min-w-[820px] text-sm">
              <thead className="bg-muted/40 text-muted-foreground border-b text-[11px] uppercase tracking-wider">
                <tr>
                  ...
                  >
                    {t('reports_hub.staff.col_churn')}
                  </th>
                  <th
                    className="px-3 py-3 text-right font-semibold"
                    title={t('reports_hub.staff.col_scoring_tooltip')}
                  >
                    {t('reports_hub.staff.col_scoring')}
                  </th>
                  <th className="px-3 py-3 text-right font-semibold">
                ...
                {rows.map((r) => {
                  ...
                  return (
                    <tr key={r.staff_id} className="border-border/60 hover:bg-muted/20 border-t">
                      ...
                      {/* bug 64d0bbf8 — Отток и Скоринг (server-side из RPC).
                          Пороги:
                            - Отток: <20% зелёный, 20-40% жёлтый, >40% красный
                            - Скоринг: >1.5 зелёный, 0.5-1.5 нейтральный, <0.5 красный */}
                      <td className="px-3 py-3 text-right">
                        <span
                          className={cn(
                            'num text-sm font-bold',
