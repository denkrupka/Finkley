...
/**
 * Обратное направление LinkTransactionDialog (credit): пользователь стоит
 * на карточке визита/продажи и выбирает банковскую credit-транзакцию для
 * привязки. Показывает только credit-tx без linked_visit_id /
 * linked_other_income_id / expense_id из окна ±90 дней.
 */
export function LinkVisitToBankDialog({
  ...
  function handlePick(txId: string) {
    link.mutate(
      { transactionId: txId, visitId: visit.id, clearNeedsReview: true },
      {
        onSuccess: () => {
        ...
        <div className="flex flex-col gap-3 px-5 pb-4 pt-1">
          ...
          <div className="border-border h-[360px] overflow-y-auto rounded-md border">
            {txsQ.isLoading ? (
              ...
              <ul>
                {items.map((tx) => {
                  const exact = tx.amount_cents === visit.amount_cents
                  const close = !exact && Math.abs(tx.amount_cents - visit.amount_cents) < 100
                  return (
